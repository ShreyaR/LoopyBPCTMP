...
def orderingVE(n1, n2, sk1, sk2, ps):

    #Constructing the Graph
    graph = getGraph(n1, n2, sk1, sk2, ps)
    #print graph
...
def getOrdering(w1, w2, a1, a2):
    sk1, sk2, ps = findingSkips(w1, w2, a1, a2)
    n1 = len(w1)
    n2 = len(w2)
    ordering = orderingVE(n1, n2, sk1, sk2, ps)

    return ordering

#Returns the clique tree after VE
def getCliqueTree(w1, w2, a1, a2):
    cliqueMapping = {}
    tree = {}
    ...
    sk1, sk2, ps = findingSkips(w1, w2, a1, a2)
    graph = getGraph(n1, n2, sk1, sk2, ps)
    ordering = getOrdering(w1, w2, a1, a2)
    #print 'ORDERING', ordering, '\n\n'
...
def forwardBackward(w1, w2, a1, a2, model):
    cliqueTree, mapping = getCliqueTree(w1, w2, a1, a2)
    ...
    while(len(queue)):
        ...
        if (len(itemsInCurrentNode) - len(termsInMessage)):
            tauFactor = {t: 1 for t in xrange(pow(10, len(termsInMessage)))}
        else:
        ...
        for i in psiFactor.keys():
            assignment = numberToAssignment(i, len(itemsInCurrentNode))
            tempAssignment = [assignment[itemsInCurrentNode.index(x)] for x in termsInMessage]
            index = assignmentToNumber(tempAssignment)
    ...
    while(len(backwardsQueue)):
        ...
        for child in childrenTree[item]:
            ...
            messageToChild = {x:log(messageToChild[x])- relevantSeparator[x] for x in messageToChild.keys()}
    ...
    return marginalAtEachNode, mapping

def getPartitionFunction(w1, w2, a1, a2, model):
    marginalAtCluster = forwardBackward(w1, w2, a1, a2, model)[0][0]
...
def getMaxMarginalAssignment(w1, w2, a1, a2, model):
    marginals,mapping = forwardBackward(w1, w2, a1, a2, model)
...
#print getMaxMarginalAssignment([405, 840, 840], [826, 623, 840], ['a', 'b', 'b'], ['f', 'g', 'b'], 4)